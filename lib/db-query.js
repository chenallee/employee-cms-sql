...
const getDeptSalaries = (id) => {
  return new Promise((resolve, reject) => {
    connection.query('SELECT role.salary FROM role WHERE department_id = ?', [id], (err, salaries) => {
      if (err){
        console.error(err);
        reject(err);
        return;
...
module.exports = {
  viewAllEmp, getEmpRoles, queryEmpRole, getManagers, queryEmpManager, getEmpShort, updateEmpRole, updateEmpManager, addNewEmp, deleteEmp, getRolesTable, getDepts, addNewRole, deleteRole, getDeptSalaries
}